...
export default function AdminBookings() {
  ...
  const exportCSV = () => {
    const headers = ['Ref,Visitor,Email,Phone,Show,Date,Tickets,Amount,Status,Time']
    const rows = filtered.map(b =>
      `${b.ref},${b.name},${b.email},${b.phone},${b.show},${b.date},${b.tickets},${b.amount},${b.status},${b.time}`
    )
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by name, ref, or show..."
            value={search}
        ...
        <button onClick={exportCSV} className="glass-light border border-white/10 text-white text-sm font-bold px-5 py-3 rounded-2xl flex items-center gap-2 hover:bg-white/10 transition-all">
          <Download size={15} /> Export CSV
        </button>
        <button onClick={fetchBookings} className="glass-light border border-white/10 text-dark-300 hover:text-white p-3 rounded-2xl transition-all">
          <RefreshCw size={15} />
        </button>
      </div>
      ...
      <div className="glass rounded-3xl border border-white/5 overflow-hidden">
        <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
          <h3 className="text-white font-bold">All Bookings</h3>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="px-6 py-12 text-center text-dark-500 text-sm">No bookings found</td></tr>
                ) : (
                  filtered.map(b => {
                      ...
                      <tr key={b.ref} className="hover:bg-white/2 transition-colors" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 btn-primary rounded-lg flex items-center justify-center text-white text-xs font-black flex-shrink-0">{b.name?.[0]?.toUpperCase()}</div>
                            <div>
